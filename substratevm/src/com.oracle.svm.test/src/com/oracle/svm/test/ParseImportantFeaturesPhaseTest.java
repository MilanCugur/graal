...
public class ParseImportantFeaturesPhaseTest extends GraalCompilerTest {
    private void testCodeSnippet(String snippet, String groundTruth) {
        ...
        for (String line : lines) {
            ...
            String NodeDescription = parts[0].trim();
        ...
        try {
            ...
            while ((line = csv.readLine()) != null) {
                if (line.equals("Graph Id,Source Function,Node Description,Node BCI,Node CALL,head,CD Depth,N. CS Father Blocks,N. CS Father Fixed Nodes,N. CS Father Floating Nodes"))  // skip header line
                    continue;
    ...
    private File __getAttributes(String snippet) {
        ...
        for (File file : files) {
            if (file.isDirectory() && file.getName().contains("database")) {
    ...
    private String __sortPath(String path) {  // sorte blocks if there are blocks in path
        if (!path.startsWith("x") && !path.equals("null"))
            return Arrays.stream(path.split(",")).map((String block) -> block.trim()).sorted().collect(Collectors.joining(", "));
        else
            return path;
    }
    ...
    private void testAttributesCodeSnippet(String snippet, String[] attributesTruth, String groundTruth) {
        ...
        try {
            ...
            while ((line = csv.readLine()) != null) {
                ...
                csData.put("CS Depth", data[6]);
                csData.put("N. CS Father Blocks", data[7]);
                csData.put("N. CS Father Fixed Nodes", data[8]);
                csData.put("N. CS Father Floating Nodes", data[9]);
                ...
                for (int index = 10; index < data.length; index++) {
                    ...
                    for (int i = 1; i < src.length; i++) {
                        ...
                        String attr = src[i];
                        sonData.put(attr.split(":")[0].trim(), attr.split(":")[1].trim());
                    }
    ...
    private static int example_Arrays(int a, int b, int[] c) throws Exception {
        ...
        // Invoke Arrays.compare: Arrays.compare deprecated in java8
        System.out.println(tmp.equals(tmp2));
